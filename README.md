...
4\. Отредактируйте файл `common/config/main-local.php`, прописав в нём правильные параметры подключения к созданной вами базе данных. Также отредактируйте `common/config/params-local.php` переопределив в нём, как минимум, параметр `photos.md5salt` - это секретная "соль" для md5, необходимая при формировании имени файла фотографии. Имя файла фотографии формируется как `md5($salt . $id)` и эта соль нужна, чтобы если вы выложите эту систему в открытый доступ, кто угодно не смог выкачать все ваши фотографии просто перебирая ID'шники. Файлы лежат на диске и отдаются как статика, т.е. без авторизации через пароль/cookie.
