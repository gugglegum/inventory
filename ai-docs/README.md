...
Известный дефект вне этого шага: кнопки удаления и сортировки фотографий на форме редактирования заметки используют общие endpoints `photo/delete`, `photo/sort-up` и `photo/sort-down`, но `backend/controllers/PhotoController.php` сейчас ищет только `ItemPhoto`. Форма заметки передает ID `PostPhoto`, поэтому сортировка и удаление фотографий постов не работают, хотя аналогичная функциональность у фотографий предметов работает.
