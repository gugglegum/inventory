...
файлы можно перетаскивать в любое место страницы с видимой формой
или вставлять из clipboard. Во время file drag глобальный listener
показывает рамку по краям viewport и подсказку; overlay не перехватывает
pointer events и поддерживает светлую/тёмную тему. Счётчик вложенных
dragenter/dragleave сохраняет индикацию при переходах между элементами;
drop, выход со страницы, Escape, dragend и blur убирают подсветку.
Общий drop-handler вызывает `addFiles()` один раз, включая сброс в прежнюю
droparea и текстовые поля. При нескольких editor-ах приоритет получает
editor под курсором, затем единственный editor формы под курсором,
затем последний активный или единственный видимый editor страницы.
Перетаскивание текста/ссылок и pointer-сортировка фотографий не перехватываются.
...
Файл можно повторить после network/server error. Кнопки сохранения остаются
активными во время загрузки. Нажатие при queued/uploading карточках или
активном DELETE временной карточки включает общее для формы ожидание:
в обеих кнопках предмета появляется spinner, повторное нажатие любой кнопки
отменяет только ожидание, а загрузки продолжаются. Новые файлы, добавленные
во время ожидания, также должны закончить загрузку перед сохранением.
Когда блокирующих операций больше нет, клиент обновляет manifest и повторяет
обычный click исходной кнопки с HTML/Yii-валидацией. Native submit и Yii
`beforeSubmit` также защищены от отправки незавершенной очереди.
Ошибка карточки снимает ожидание: нужно повторить загрузку или убрать
проблемную карточку, затем снова сохранить форму. Поведение общее для
предметов и заметок.

Браузерная регрессия: `node --test app/tests/js/photo-editor.browser.cjs`
(нужны установленный npm-пакет `playwright` и Chromium; путь к системному
браузеру можно задать через `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH`). Тесты
используют настоящие jQuery/Yii assets и editor JS с изолированной формой
и управляемыми ответами upload/session/delete, без рабочих данных.
...
Обычное поле поиска `q` ищет слова только по репозиторному `item.itemId`,
названию и тегам. Глобальный `item.id` не является пользовательским номером и
используется только как внутренний ключ связей; описание предмета намеренно не
входит в обычный запрос. Сворачиваемый блок расширенного поиска передает
отдельные GET-параметры `description` и `notes`. Непустые группы объединяются
через `AND`; `notes` ищет одновременно по `post.title` и `post.text` через
коррелированный `EXISTS`, причем все позитивные слова должны встретиться в одной
заметке. Негативное слово заметок исключает предмет, если оно найдено хотя бы в
одной его заметке. Такой подзапрос сохраняет одну строку результата на предмет
при любом количестве тегов и заметок.

Поля расширенного поиска disabled, пока блок `<details>` закрыт. Клиентский
`backend/web/js/search-form.js` синхронизирует disabled-состояние при раскрытии,
сворачивании и восстановлении страницы из истории браузера. Пустые текстовые
критерии удаляются из события `formdata`, без изменения состояния элементов
...
неизвестный callback не уничтожает flows других вкладок. С 2026-09-13 отдельный ограничитель частоты SSO-входов в Stockhub удалён,
чтобы поведение совпадало с Book, Memo, Photo, Project, Wiki, YouTube и CarServe.
Начало authorization flow и callback больше не расходуют локальные IP/deployment
квоты. Удалён `OidcFlowRateLimiter`; его старые runtime JSON/lock-файлы больше
не используются. Проверки одноразового state, nonce, PKCE и токена сохраняются.

Общие ограничения остаются на стороне Pyrda SSO: по умолчанию `/oauth/token`
допускает 10 запросов в минуту на client_id + IP сервера приложения и 60 на IP.
Эти квоты учитывают успешные запросы и разделяются пользователями сайта;
удаление локального limiter не отменяет возможный отказ самого провайдера.
Лимиты SSO этой правкой не изменялись. Регрессионный integration-тест выполняет
12 последовательных успешных входов/выходов с одного IP с подменённым OIDC
transport, проверяя отсутствие прежних ограничений начала входа и callback.
